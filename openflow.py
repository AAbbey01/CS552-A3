...
class Tutorial (object):
    ...
    def act_like_switch(self, packet, packet_in):
        ...
        """
        print ("Packet from ",str(packet.src),"to", str(packet.dst)  )
        ...
        if packet.dst in self.mac_to_port:
            ...
            print(f"path known from {packet.src} to {packet.dst} via port {out_port}")
            # Install flow rule for future packets of this flow
            # Create a flow modification message
            fm = of.ofp_flow_mod()
            
            # Match packets with the same destination MAC address
            fm.match.dl_dst = packet.dst
            
            # Set the output action to forward to the correct port
            action = of.ofp_action_output(port=out_port)
